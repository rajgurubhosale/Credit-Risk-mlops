...
class NumericalWOEBinner:
    ...
    def _manual_prebinning(self,
       ...
       - Returns:
        ...
        max_possible_bins = normal_series.shape[0] / data_per_bin
        max_possible_bins = int(np.floor(max_possible_bins))
        ...
        # assign 0 woe where event or non event is 0 count  more stable woe values
        bin_table["WoE"] = np.where(
            (bin_table["Event"] == 0) | (bin_table["Non-event"] == 0),
            0,  # Assign WoE=0 for rare bins 
        ...
        # Sort normal bins naturally
        normal_bins = normal_bins.sort_values("Bin")
        normal_bins = normal_bins.copy()
        normal_bins["lower"] = normal_bins["Bin"].apply(lambda x: float(str(x).split(",")[0].replace("(", "").replace("[","")))
        ...
        "Event rate": [bin_table["Event"].sum() / bin_table["Count"].sum() if bin_table["Count"].sum() != 0 else 0],
        "WoE": [None],
        "IV": [bin_table["IV"].sum()],
         }, index=["Totals"])
        ...
        return final_bins, bin_table, bin_edges.tolist(), total_iv
    ...
    def fit(self, X, y):
        ...
        for feature in self.numerical_features:
            try:
                bins, summary, edges, total_iv = self._manual_prebinning(
                    df=X,
                    feature=feature,
                    target=y,
    ...
    def transform(self, X):
        ...
        for feature in self.numerical_features:
            ...
            summary = self.bin_summary_[feature]
            woe_map = dict(zip(summary["Bin"], summary["WoE"]))

            # Clip values to training min/max (nearest bin)
            series_clipped = series.clip(lower=edges[0], upper=edges[-1])

            # Assign bins using training edges
            binned = pd.cut(series_clipped, bins=edges, include_lowest=True).astype(str)
            ...
            # Map bins to WOE
            woe_data[feature] = binned.map(woe_map)
...
class CategoricalWOEBinner:
    ...
    def fit(self, X_train_cat, y_train):
        ...
        for col in self.categorical_features:
            ...
            # WOE
            agg["woe"] = np.where(
                (agg["good"] == 0) | (agg["bad"] == 0),
                0,  # Assign 0 for rare bins / zero counts
...
class FeatureSelector:
    ...
    def correlation_filter(self, X_train_woe, keep_features, iv_df):
        ...
        try:
            ...
            # Ensure iv_df index is feature names for proper alignment
            iv_df = iv_df.copy()
            iv_df.index = iv_df['feature']

            # Keep only features present in both X_filtered_woe and iv_df
            ...
            for feature in upper.columns:
                ...
                # Find correlated features above threshold
                ...
                # Keep the feature with highest IV
...
class FetureEngineeringPipeline:
    def __init__(self, id_col, target_col):
        self.id_col = id_col
        self.target_col = target_col
    ...
    def _feature_selection(self, X_train_woe, X_test_woe, X_train):

        self.fe_selector = FeatureSelector(id_col=self.id_col)
    ...
    def run(self):
        try:
            ...
            X_selected_train, X_selected_test = self._feature_selection(X_train_woe,X_test_woe,X_train)
...
if __name__ == '__main__':
    fe_pipeline = FetureEngineeringPipeline(id_col,target_col)
    fe_pipeline.run()
